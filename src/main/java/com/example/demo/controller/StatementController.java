...
public class StatementController extends BaseController {
    ...
    @GetMapping("/bet/settled")
    public Result settled(@RequestParam String websiteId, @RequestParam String accountId, @RequestParam(value = "date", required = false) String date) {
        if ("undefined".equals(websiteId) || "undefined".equals(accountId)) {
            return Result.success();
        }
        AdminLoginDTO admin = getUser();
