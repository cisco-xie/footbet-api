...
public class WebsiteXinBaoBetPreviewHandler implements ApiHandler {
    ...
    @Autowired
    public WebsiteXinBaoBetPreviewHandler(OkHttpProxyDispatcher dispatcher, WebsiteService websiteService, ApiUrlService apiUrlService) {
    ...
    public String buildRequest(JSONObject params) {
        ...
        // 构造请求体
        return String.format("p=FT_order_view&uid=%s&ver=%s&langx=zh-cn&odd_f_type=%s&gid=%s&gtype=%s&wtype=%s&chose_team=%s",
                params.getStr("uid"),
                Constants.VER,
                oddsFormatType,
                gid,
    ...
    public JSONObject parseResponse(JSONObject params, OkHttpProxyDispatcher.HttpResult response) {
        ...
        result.putOpt("data", serverresponse);
