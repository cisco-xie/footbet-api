...
@Slf4j
@Component
public class WebsitePingBoKeepAliveHandler implements ApiHandler {
    ...
    @Autowired
    public WebsitePingBoKeepAliveHandler(OkHttpProxyDispatcher dispatcher, WebsiteService websiteService, ApiUrlService apiUrlService) {
...
//        headers.put("v-hucode", params.getStr("hucode"));
    ...
    public JSONObject parseResponse(JSONObject params, OkHttpProxyDispatcher.HttpResult result) {
        ...
        if (result == null || result.getStatus() != 200) {
            ...
            res.putOpt("msg", status == 403 ? "账户保活失效" : "账户保活失败");
            return res;
        }
        List<String> hucodes = result.getHeaders().get("v-hucode");
        // 解析响应
        JSONObject responseJson = new JSONObject(result.getBody());
        if (hucodes.isEmpty()) {
            responseJson.putOpt("success", false);
            responseJson.putOpt("msg", "账户保活请求失败");
            return responseJson;
        }

        responseJson.putOpt("success", true);
        responseJson.putOpt("hucode", hucodes.get(0));
        responseJson.putOpt("durationMs", result.getDurationMs());
        responseJson.putOpt("msg", "账户保活成功");
        return responseJson;
    }
    ...
    public JSONObject execute(ConfigAccountVO userConfig, JSONObject params) {
        ...
        String apiUrl = apiUrlService.getApiUrl(siteId, "keepalive");
        // 构建请求
        Map<String, String> requestHeaders = buildHeaders(params);
        String queryParams = buildRequest(params);
        ...
        try {
            ...
            log.error("请求异常，用户:{}, 账号:{}, 参数:{}, 错误:{}", username, userConfig.getAccount(), queryParams, e.getMessage(), e);
