...
@Slf4j
@Component
public class WebsiteXinBaoChangePwdHandler implements ApiHandler {
    ...
    @Autowired
    public WebsiteXinBaoChangePwdHandler(OkHttpProxyDispatcher dispatcher, WebsiteService websiteService, ApiUrlService apiUrlService) {
    ...
    public String buildRequest(JSONObject params) {
        ...
        String chgPassword = params.getStr("chgPassword");
        return String.format("p=chg_newpwd&uid=%s&ver=%s&langx=zh-cn&username=%s&new_password=%s&chg_password=%s",
                params.getStr("uid"),
                Constants.VER,
                username,
                newPassword,
    ...
    public JSONObject parseResponse(JSONObject params, OkHttpProxyDispatcher.HttpResult response) {
        ...
        try {
            ...
            log.error("[新2][修改失败][JSON解析异常][body={}]", responseBody, e);
            return new JSONObject()
